...
public class ControllerThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			logger.info("{} Starting to get Server detail for {} CSA Nodes", Common.getLogHeader(this, "run"), controllerService.listCSANode.size());

			int totalServersChecked = 0;
			for (String serverName : controllerService.listCSANode.keySet()) {
				if (controllerService.listServers.containsKey(serverName) && !controllerService.listServersLicensed.containsKey(serverName)) {
					totalServersChecked += 1;

					logger.debug("{} Looking detail for server {}, total servers until now [{}] from [{}]", Common.getLogHeader(this, "run"), serverName,
					...
					if (totalServersChecked % 100 == 0) {
						logger.info("{} Total servers until now [{}] from [{}]", Common.getLogHeader(this, "run"), totalServersChecked,
